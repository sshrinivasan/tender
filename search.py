...
def _extract_tender(doc) -> dict:
    ...
    if source == "merx":
        org = m.get("buyer", "")
        url = m.get("page_url", "")
    elif source in ("procuredata", "bidsandtenders"):
...
def run_search(query: str, source: str = "all", closing_days: int | None = None, regions: list[str] | None = None) -> dict:
    source_filter = ["merx", "canadabuys", "procuredata", "bidsandtenders"] if source == "all" else [source]
